...
\geometry{a4paper, left=20mm, right=20mm, top=20mm, bottom=20mm,showframe=true}
...
\begin{document}
    ...
    \begin{tabular}{ p{2cm} p{7cm} p{3cm} r r}
        name & problem & solution & author & P\\
      \hline
        Checking Change 
        & \textit{Given a number of different coin-values $c_i$, 
            output the minimum number of coins that are necessary to represent $m_i$.} 
        & DP       & Ben    & \pageref{sec:checking_change} \\

        Dominoes 
        & \textit{Given a list of tiles of different heights, determine how many tiles will
            fall after toppling the left-most tile.} 
        & trivial & Ben & \ref{sec:dominoes} \\

        Shelves 
        & \textit{Given two different types of shelves with length $m$ and $n$, $m\leq n$ and an empty space with length $l$,
        ...
        Even Pairs 
        & \textit{Given a list $x_1,\dots,x_n$, count the number of pairs $1\leq i\leq j \leq n$ for which 
            the sum is even.} 
        & DP & Jonas & \ref{sec:even_pairs} \\



                % & \textit{Given a set of intervals...} & Aliens & Ben & \ref{sec:aliens} \\
